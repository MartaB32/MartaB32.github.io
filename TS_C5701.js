...
var Application = function() {
	...
	var KOSZYK_KEY = 'koszyk';
	self.dejKoszyk = function() {
		var koszyk = localStorage.getItem(KOSZYK_KEY);
		if (!koszyk) {
			return [];
		} else {
			return JSON.parse(koszyk);
		}
	}

	self.saveKoszyk = function(koszyk) {
		localStorage.setItem(KOSZYK_KEY, JSON.stringify(koszyk));
	}

	self.dodajRzeczDoKoszyka = function(nazwaRzeczy, cenaRzeczy) {
		var rzeczObj = {
			nazwa: nazwaRzeczy,
			cena: cenaRzeczy
		}
		console.log(rzeczObj)
		var zapisanyKoszyk = self.dejKoszyk();
		zapisanyKoszyk.push(rzeczObj);
		self.saveKoszyk(zapisanyKoszyk);
	}
	
	self.wyczyscKoszyk = function() {
		localStorage.removeItem(KOSZYK_KEY);
	}
	
	self.usunRzeczZKoszyka = function(indeks) {
		var zapisanyKoszyk = self.dejKoszyk();
		zapisanyKoszyk.splice(indeks, 1);
		self.saveKoszyk(zapisanyKoszyk);
		window.location.reload();
	}
	
	self.dejHTMLKoszyka = function() {
		var zapisanyKoszyk = self.dejKoszyk();
		var dupa = '';
		for(var i = 0; i < zapisanyKoszyk.length; i++) {
			dupa = dupa + '<h3>Nazwa: </h3>' + zapisanyKoszyk[i].nazwa + '<br>';
			dupa = dupa + '<h3>Cena: </h3>' + zapisanyKoszyk[i].cena + '<br>';
			dupa = dupa + '<div onclick=\"application.usunRzeczZKoszyka(' + i + ')\"><span>Wywal to</span></div><br>';
		}
		dupa = '<br><h3>Produkty: </h3></br>'+dupa;
		return dupa;
	}
	
	self.dejSumeKoszyka = function() {
		var zapisanyKoszyk = self.dejKoszyk();
		var suma = 0;
		for(var i = 0; i < zapisanyKoszyk.length; i++) {
			if (zapisanyKoszyk[i].cena && typeof zapisanyKoszyk[i].cena === 'number') {
				suma = suma + zapisanyKoszyk[i].cena;
			}
		}
		suma='<br><br><h3>Suma: </h3><br>'+suma+' zł<br>';
		return suma;
	}

	var DANE_KEY = 'dane';

	self.dejDane = function() {
		var dane = localStorage.getItem(DANE_KEY);
	...
	self.saveDane = function(dane) {
		localStorage.setItem(DANE_KEY, JSON.stringify(dane));
	}

	self.zbierDane = function() {
		var dane = {
			name: document.getElementById('form_name').value,
			surname: document.getElementById('form_surname').value,
			telephone: document.getElementById('form_phone_number').value,
	...
	self.onSaveDaneClick = function() {
		self.saveDane(self.zbierDane());
	}

	self.dejHTMLDanych = function() {
		console.log('aaaa');
		var dane = self.dejDane();
		console.log('dane', dane);
		var dupa = '';
		dupa = dupa + '<h3> Imie: </h3>' + dane.name + '<br>';
		dupa = dupa + '<h3> Nazwisko: </h3>' + dane.surname + '<br>';
		dupa = dupa + '<h3> Telefon: </h3>' + dane.telephone + '<br>';
		dupa = dupa + '<h3> Email: </h3>' + dane.email + '<br>';
		dupa = dupa + '<h3> Adres: </h3>' + dane.address + '<br>';
		dupa='<br><h3>Dane: </h3><br>'+dupa;
		return dupa;
	}

	self.restoreDane = function() {
		var dane = self.dejDane();
		if (dane.name) {
			document.getElementById('form_name').value = dane.name;
			document.getElementById('form_surname').value = dane.surname;
			document.getElementById('form_phone_number').value = dane.telephone;
			document.getElementById('form_mail').value = dane.email;
			document.getElementById('form_address').value = dane.address;
		}
	}

	self.wyczyscDane = function() {
		localStorage.removeItem(DANE_KEY);
	}

	self.shoppingDone = function() {
		self.wyczyscKoszyk();
		self.wyczyscDane();
	}
